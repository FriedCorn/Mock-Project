...
function addQuestion(quiz, number) {
    ...
    let question__list_answer = createListAnswer(quiz["answers"], quiz["id"]);
...
$(document).ready(function() {
    $(".btn__remove").click(function() {
        ...
        $(document).ready(function() {
            $.ajax({
                url: '/quiz/' + list_quiz_id + "/delete/" + this.parentElement.id,
                type: 'DELETE',
                success: function(data) {
                    console.log(data);
                    loadData(data);
